...
contract Person {
    ...
    function getSalary() external  view returns (uint16);
...
contract Researcher is Person{
    ...
    uint16 _researchSalary,string memory _researchArea, uint16 _numberOfPublications) public
    {
     ...
     function getSalary() external onlyOwner view returns (uint16){
...
contract Teacher is Person{
    ...
    function changeCourse(string calldata newCourseName,uint16 newNumberOfStudents) external onlyOwner{
